...
// Function to open multiple indices
const openIndices = async (indices) => {
  try {
    for (const index of indices) {
      console.log(`Attempting to open index: ${index}`);
      
      // Open the index
      const { body } = await client.indices.open({ index });
      
      console.log(`Index '${index}' opened successfully:`, body);
    }
  } catch (error) {
    console.error(`Error opening index:`, error.meta?.body?.error || error);
  }
};

// Replace this with the array of indices you want to open
const indicesToOpen = [
...
if (indicesToOpen.length === 0) {
  console.error('Please provide at least one index to open.');
  process.exit(1);
}

openIndices(indicesToOpen);
